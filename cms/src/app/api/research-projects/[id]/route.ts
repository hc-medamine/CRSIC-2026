...
function serialize(item: NonNullable<Awaited<ReturnType<typeof getResearchProjectById>>>) {
  return {
    ...item,
  ...
  try {
    ...
    switch (body.action) {
      case "save":
        ...
        item = await updateResearchProjectDraft(user, id, body.fields);
        break;
      case "submit":
        if (body.fields) {
          await updateResearchProjectDraft(user, id, body.fields);
        }
        item = await submitResearchProject(user, id, Boolean(body.checklistConfirmed));
        break;
      case "withdraw":
        item = await withdrawResearchProject(user, id);
        break;
      case "request_changes":
        item = await requestResearchProjectChanges(user, id, body.note ?? "");
        break;
      case "approve":
        item = await approveResearchProject(user, id);
        break;
      case "reject":
        item = await rejectResearchProject(user, id, body.note ?? "");
        break;
      case "publish":
        item = await publishResearchProject(user, id);
        break;
      case "unpublish":
        item = await unpublishResearchProject(user, id);
        break;
      case "start_revision":
