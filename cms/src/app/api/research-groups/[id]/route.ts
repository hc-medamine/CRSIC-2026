...
import {
  ...
  rejectResearchGroup,
  requestResearchGroupChanges,
  submitResearchGroup,
  unpublishResearchGroup,
  updateResearchGroupDraft,
  withdrawResearchGroup,
  type ResearchGroupInput,
...
function serialize(item: NonNullable<Awaited<ReturnType<typeof getResearchGroupById>>>) {
  return {
    ...item,
  ...
  try {
    ...
    switch (body.action) {
      case "save":
        ...
        item = await updateResearchGroupDraft(user, id, body.fields);
        break;
      case "submit":
        if (body.fields) {
          await updateResearchGroupDraft(user, id, body.fields);
        }
        item = await submitResearchGroup(user, id, Boolean(body.checklistConfirmed));
        break;
      case "withdraw":
        item = await withdrawResearchGroup(user, id);
        break;
      case "request_changes":
        item = await requestResearchGroupChanges(user, id, body.note ?? "");
        break;
      case "approve":
        item = await approveResearchGroup(user, id);
        break;
      case "reject":
        item = await rejectResearchGroup(user, id, body.note ?? "");
        break;
      case "publish":
        item = await publishResearchGroup(user, id);
        break;
      case "unpublish":
        item = await unpublishResearchGroup(user, id);
        break;
      case "start_revision":
        await startRevision(user, id);
        item = await getResearchGroupById(id);
        break;
