...
function serialize(item: NonNullable<Awaited<ReturnType<typeof getPartnerById>>>) {
  return {
    ...item,
  ...
  try {
    ...
    switch (body.action) {
      case "save":
        ...
        item = await updatePartnerDraft(user, id, body.fields);
        break;
      case "submit":
        if (body.fields) {
          await updatePartnerDraft(user, id, body.fields);
        }
        item = await submitPartner(user, id, Boolean(body.checklistConfirmed));
        break;
      case "withdraw":
        item = await withdrawPartner(user, id);
        break;
      case "request_changes":
        item = await requestPartnerChanges(user, id, body.note ?? "");
        break;
      case "approve":
        item = await approvePartner(user, id);
        break;
      case "reject":
        item = await rejectPartner(user, id, body.note ?? "");
        break;
      case "publish":
        item = await publishPartner(user, id);
        break;
      case "unpublish":
        item = await unpublishPartner(user, id);
        break;
      case "start_revision":
        await startRevision(user, id);
        item = await getPartnerById(id);
        break;
