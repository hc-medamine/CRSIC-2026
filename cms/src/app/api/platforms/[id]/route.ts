...
function serialize(item: NonNullable<Awaited<ReturnType<typeof getPlatformById>>>) {
  return {
    ...item,
...
export async function GET(_request: NextRequest, { params }: Params) {
  ...
  const item = await getPlatformById(id);
  ...
  try {
    ...
    switch (body.action) {
      ...
      case "submit":
        ...
        break;
      case "withdraw":
        item = await withdrawPlatform(user, id);
        break;
      case "request_changes":
        item = await requestPlatformChanges(user, id, body.note ?? "");
        break;
      case "approve":
        item = await approvePlatform(user, id);
        break;
      case "reject":
        item = await rejectPlatform(user, id, body.note ?? "");
        break;
      case "publish":
        item = await publishPlatform(user, id);
        break;
      case "unpublish":
        item = await unpublishPlatform(user, id);
        break;
      case "start_revision":
        await startRevision(user, id);
        item = await getPlatformById(id);
        break;
