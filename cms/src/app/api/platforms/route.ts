...
import { createPlatform, listPlatformsForUser, type PlatformInput } from "@/lib/content/platforms";
...
function serialize(item: Awaited<ReturnType<typeof listPlatformsForUser>>[number]) {
  return {
    ...item,
...
export async function POST(request: NextRequest) {
  ...
  if (!user) return NextResponse.json({ ok: false, error: "Unauthenticated" }, { status: 401 });
  try {
    const body = (await request.json()) as PlatformInput;
    const item = await createPlatform(user, body);
