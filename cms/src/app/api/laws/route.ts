...
import { createLaw, listLawsForUser, type LawInput } from "@/lib/content/laws";
...
function serialize(item: Awaited<ReturnType<typeof listLawsForUser>>[number]) {
  return {
    ...item,
...
export async function POST(request: NextRequest) {
  ...
  if (!user) return NextResponse.json({ ok: false, error: "Unauthenticated" }, { status: 401 });
  try {
    const body = (await request.json()) as LawInput;
    const item = await createLaw(user, body);
