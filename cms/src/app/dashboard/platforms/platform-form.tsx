...
export function PlatformEditorForm({
  mode,
  orgUnits,
  initial,
  canSubmit,
  ...
  const [attachments, setAttachments] = useState<PublicMediaItem[]>(() => {
    ...
    if (initial?.imagePath) {
      return [{ kind: "image", src: initial.imagePath }];
    }
    return [];
  });
  ...
  const [platformKind, setPlatformKind] = useState<"visual" | "radio" | "mobility">(initial?.platformKind ?? "visual");
  ...
  async function create(e: FormEvent) {
    ...
    try {
      ...
      router.push(`/dashboard/platforms/${data.item.id}`);
      router.refresh();
    } finally {
      setPending(false);
    }
  ...
  async function run(action: string, extra?: Record<string, unknown>) {
    ...
    setMessage(null);
    try {
      const res = await fetch(`/api/platforms/${initial.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        <FormSection step={1} title={t("sectionIdentity", lang)}>
          ...
          <label className="text-sm">
            ...
            <textarea dir="rtl" disabled={!editable} value={summaryAr} onChange={(e) => setSummaryAr(e.target.value)} className="mt-1 w-full min-h-11 rounded-xl border border-crs-border bg-crs-surface px-3 py-2 text-sm text-crs-ink" rows={2} />
          </label>
          <label className="text-sm">
            <span className="font-medium">{t("fieldPlatformKind", lang)}</span>
            <select disabled={!editable} value={platformKind} onChange={(e) => setPlatformKind(e.target.value as "visual" | "radio" | "mobility")} className="mt-1 w-full min-h-11 rounded-xl border border-crs-border bg-crs-surface px-3 py-2 text-sm text-crs-ink">
              <option value="visual">{t("platformKindVisual", lang)}</option>
              <option value="radio">{t("platformKindRadio", lang)}</option>
              <option value="mobility">{t("platformKindMobility", lang)}</option>
            </select>
          </label>
          <label className="text-sm">
            <span className="font-medium">{t("fieldExternalUrl", lang)}</span>
            <input dir="ltr" disabled={!editable} value={externalUrl} onChange={(e) => setExternalUrl(e.target.value)} className="mt-1 w-full min-h-11 rounded-xl border border-crs-border bg-crs-surface px-3 py-2 text-sm text-crs-ink" placeholder="https://" />
          </label>
        </FormSection>
        ...
        <FormSection step={3} title={t("sectionMedia", lang)}>
          <MediaUploadField
            ...
            onUploaded={({ publicPath }) => {
              ...
              setAttachments((prev) => {
                const rest = prev.filter((a) => a.src !== publicPath);
                return [{ kind: "image", src: publicPath }, ...rest];
              });
            }}
          />
        ...
        <AdvancedDisclosure step={4} title={t("sectionAdvanced", lang)} hint={t("sectionAdvancedHint", lang)}>
          <label className="text-sm">
          ...
          <label className="text-sm">
            ...
            <textarea disabled={!editable} value={summaryEn} onChange={(e) => setSummaryEn(e.target.value)} className="mt-1 w-full min-h-11 rounded-xl border border-crs-border bg-crs-surface px-3 py-2 text-sm text-crs-ink" rows={2} />
          </label>
          <RichBodyEditor
          ...
          <SeoFieldsSection
            ...
            disabled={!editable}
            ogBucket="platforms"
            ogFallbackHint={imagePath.trim() || "img/cms/platforms/..."}
        ...
        <FormStickyActions>
          <div className="flex w-full flex-wrap items-center justify-end gap-2">
            {mode === "create" ? (
              <button type="submit" disabled={pending} className="inline-flex min-h-11 items-center rounded-xl bg-crs-primary px-4 py-2 text-sm font-medium text-white hover:bg-crs-secondary disabled:opacity-60">
                {pending ? t("actionSaving", lang) : t("actionCreateDraft", lang)}
              </button>
            ) : null}
            {mode === "edit" && editable && isAuthor ? (
              <>
                <button type="button" disabled={pending} className="inline-flex min-h-11 items-center rounded-xl border border-crs-border bg-crs-surface px-4 py-2 text-sm text-crs-ink hover:bg-crs-bg disabled:opacity-60" onClick={() => void run("save", { fields: fields() })}>
                ...
                {canSubmit ? (
                  <button type="button" disabled={pending || !checklist} className="inline-flex min-h-11 items-center rounded-xl bg-crs-primary px-4 py-2 text-sm font-medium text-white hover:bg-crs-secondary disabled:opacity-60" onClick={() => void run("submit", { checklistConfirmed: checklist })}>
                    {t("actionSubmit", lang)}
                  </button>
                ) : null}
              </>
            ...
            {mode === "edit" && initial?.status === "submitted" && isAuthor ? (
              <button type="button" disabled={pending} className="inline-flex min-h-11 items-center rounded-xl border border-crs-border bg-crs-surface px-4 py-2 text-sm text-crs-ink hover:bg-crs-bg" onClick={() => void run("withdraw")}>
      ...
      {mode === "edit" && initial?.id ? <PublicPreviewButton contentId={initial.id} disabled={pending} /> : null}
      {mode === "edit" && canReview && (initial?.status === "approved" || initial?.status === "unpublished") ? (
        <button type="button" disabled={pending} className="w-fit rounded bg-crs-primary px-4 py-2 text-sm text-white" onClick={() => void run("publish")}>{t("actionPublish", lang)}</button>
      ) : null}
      {mode === "edit" && (isAuthor || canReview) && initial?.status === "published" ? (
        <div className="flex flex-wrap gap-2">
          <button type="button" disabled={pending} className="w-fit rounded border border-crs-secondary/40 px-4 py-2 text-sm text-crs-primary" onClick={() => void run("start_revision")}>{t("actionStartRevision", lang)}</button>
          {canReview ? (
            <button type="button" disabled={pending} className="w-fit inline-flex min-h-11 items-center rounded-lg border border-crs-border bg-crs-surface px-4 py-2 text-sm text-crs-ink hover:bg-crs-bg" onClick={() => void run("unpublish")}>{t("actionUnpublish", lang)}</button>
          ) : null}
        </div>
      ) : null}
