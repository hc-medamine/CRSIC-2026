  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          <div className="flex w-full flex-wrap items-center justify-end gap-2">
            ...
            {mode === "edit" && editable && isAuthor ? (
              <>
                ...
                {canSubmit ? (
                  <button type="button" disabled={pending || !checklist} className="inline-flex min-h-11 items-center rounded-xl bg-crs-primary px-4 py-2 text-sm font-medium text-white hover:bg-crs-secondary disabled:opacity-60" onClick={() => void run("submit", { checklistConfirmed: checklist, fields: fields() })}>
                    {t("actionSubmit", lang)}
                  </button>
                ) : null}
              </>
      ...
      {mode === "edit" && canReview && (initial?.status === "approved" || initial?.status === "unpublished") ? (
        <PublishButton pending={pending} onClick={() => void run("publish")}>{t("actionPublish", lang)}</PublishButton>
      ) : null}
