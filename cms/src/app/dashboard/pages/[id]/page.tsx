...
import { PageEditorForm } from "../page-form";
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-4xl flex-col gap-6 px-6 py-12 font-sans">
      <header className="flex items-center justify-between border-b border-zinc-200 pb-4">
        <div>
          <p className="text-sm uppercase tracking-wide text-zinc-500">Pages</p>
          <h1 className="text-2xl font-semibold">Edit / review</h1>
        </div>
        <Link href="/dashboard/pages" className="text-sm underline">
          ← Back
        </Link>
      </header>
      ...
      {item.status === "approved" || item.status === "unpublished" ? (
        <p className="rounded border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-900">
          Only one published row per page is allowed — publishing fails if another draft/approved copy of the same
          page is already published; unpublish it first.
        </p>
      ) : null}
      ...
      <RevisionHistory contentItemId={item.id} contentType="page" canRestore={canManage} />
    </main>
  );
}
