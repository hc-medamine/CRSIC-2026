...
export function LawEditorForm({
  ...
  async function create(e: FormEvent) {
    ...
    try {
      ...
      router.push(`/dashboard/laws/${data.item.id}`);
      router.refresh();
    } finally {
      setPending(false);
    }
  ...
  async function run(action: string, extra?: Record<string, unknown>) {
    ...
    setMessage(null);
    try {
      const res = await fetch(`/api/laws/${initial.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      >
        ...
        <AdvancedDisclosure step={4} title={t("sectionAdvanced", lang)} hint={t("sectionAdvancedHint", lang)}>
          ...
          <label className="text-sm">
            ...
            <input dir="ltr" disabled={!editable} value={externalUrl} onChange={(e) => setExternalUrl(e.target.value)} className="mt-1 w-full min-h-11 rounded-xl border border-crs-border bg-crs-surface px-3 py-2 text-sm text-crs-ink" placeholder="https:// (optional)" />
          </label>
          <label className="text-sm">
          ...
          <SeoFieldsSection
            ...
            disabled={!editable}
            ogBucket="laws"
            ogFallbackHint={imagePath.trim() || "img/cms/laws/..."}
