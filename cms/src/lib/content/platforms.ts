...
import { buildPlatformPayloadForItem, rebuildPublicPlatformsJson } from "@/lib/publish/platformsJson";
...
function snapshotOf(row: PlatformItem) {
  return {
    status: row.status,
...
function validatePlatformFields(input: PlatformInput) {
  if (!input.titleAr.trim()) throw new Error("Platform title (AR) is required");
  if (!["visual", "radio", "mobility"].includes(input.platformKind)) {
...
export async function listPlatformsForUser(user: SessionUser): Promise<PlatformItem[]> {
...
export async function createPlatform(user: SessionUser, input: PlatformInput): Promise<PlatformItem> {
  if (!(await canAccessContentType(user, "platform"))) throw new Error("No platform content permission");
  if (!(await canAccessOrg(user, input.orgUnitId))) throw new Error("No permission for this organisation unit");
  await assertOrgAllowsContentType(input.orgUnitId, "platform");
  validatePlatformFields(input);
  ...
  await auditPlatform(user, "platform.create", item);
  return item;
}
...
async function notifyReviewers(itemId: string, title: string, body: string, linkPath: string) {
  await notifyOnSubmit(itemId, title, body, linkPath, "platform.submitted");
}
...
export async function submitPlatform(user: SessionUser, id: string, checklistConfirmed: boolean) {
  ...
  await notifyReviewers(
    item.id,
    "Platform submitted for review",
    item.title_ar,
    `/dashboard/platforms/${item.id}`,
  );
  await auditPlatform(user, "platform.submit", item);
  return item;
}

export async function withdrawPlatform(user: SessionUser, id: string) {
  const existing = await getPlatformById(id);
  if (!existing) throw new Error("Not found");
  ...
  await auditPlatform(user, "platform.withdraw", item);
  return item;
}

async function assertReviewer(user: SessionUser, item: PlatformItem) {
...
export async function requestPlatformChanges(user: SessionUser, id: string, note: string) {
  const existing = await getPlatformById(id);
  if (!existing) throw new Error("Not found");
  ...
  await createNotification({
    ...
    type: "platform.changes_requested",
    title: "Changes requested on law",
    body: note.trim(),
    linkPath: `/dashboard/platforms/${item.id}`,
  });
  await auditPlatform(user, "platform.changes_requested", item, note.trim());
  return item;
}

export async function approvePlatform(user: SessionUser, id: string) {
  const existing = await getPlatformById(id);
  if (!existing) throw new Error("Not found");
  await assertReviewer(user, existing);
  if (existing.status !== "submitted") throw new Error("Item is not awaiting review");
  ...
  await createNotification({
    userId: item.created_by,
    type: "platform.approved",
    title: "Platform approved",
    body: item.title_ar,
    linkPath: `/dashboard/platforms/${item.id}`,
  });
  await auditPlatform(user, "platform.approve", item);
  return item;
}

export async function rejectPlatform(user: SessionUser, id: string, note: string) {
  const existing = await getPlatformById(id);
  if (!existing) throw new Error("Not found");
  await assertReviewer(user, existing);
  if (existing.status !== "submitted") throw new Error("Item is not awaiting review");
  ...
  await createNotification({
    userId: item.created_by,
    type: "platform.rejected",
    title: "Platform rejected",
    body: note.trim(),
    linkPath: `/dashboard/platforms/${item.id}`,
  });
  await auditPlatform(user, "platform.reject", item, note.trim());
  return item;
}

/**
 * Publish a platform item and rebuild data/platforms.json (multiple platforms may be live).
 */
export async function publishPlatform(user: SessionUser, id: string) {
  const existing = await getPlatformById(id);
  if (!existing) throw new Error("Not found");
  await assertReviewer(user, existing);
  ...
  const payload = await buildPlatformPayloadForItem({ ...existing, public_slug: slug });
  ...
  await createNotification({
    userId: item.created_by,
    type: "platform.published",
    title: "Platform published",
    body: item.title_ar,
    linkPath: `/dashboard/platforms/${item.id}`,
  });
  await auditPlatform(user, "platform.publish", item, "Published to platforms.json");
  return item;
}
