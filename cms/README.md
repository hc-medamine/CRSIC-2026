...
4. Open [http://localhost:3000/login](http://localhost:3000/login). Sign in with the Super Admin **email** (no SMTP — email is the login id only).
   - **Dev login bubbles:** a dark strip at the **bottom** of `/login` (not inside the sign-in card). Shown automatically in `npm run dev`; never in production. SA/Reviewer from DB; editors from `EDITOR1_EMAIL`…`EDITORN_EMAIL` sharing `CMS_LOGIN_BUBBLE_EDITOR_PASSWORD`. Set `NEXT_PUBLIC_CMS_LOGIN_BUBBLES=0` to hide locally.
