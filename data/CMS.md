...
| `featured-news.json` | `{ "items": { "type": "news"\|"event", "id": string }[], "ids": string[] }` — Home `#home-feat-carousel` playlist (max 10). `ids` is legacy news-only mirror. Empty / all missing → SPA merges newest news+events **with a cover image** by date (up to 10). CMS: `/dashboard/featured-news` |
