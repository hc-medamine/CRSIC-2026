...
- Home news pages through **all** items in the public list, **six cards per page**, looping.
- Preserve today’s **3-column × 2-row** grid on desktop (existing wrap: 2-up tablet, 1-up phone). Card design unchanged (including byline).
...
| Role | Needs |
|------|--------|
| Public visitor | See more than six Home stories without opening `#news`; pause to read; swipe on touch |
| Visitor with `prefers-reduced-motion` | Static first six + View all; no rotation, no pause button |
...
2. **Paging:** `pageCount = Math.ceil(n / 6)`. Page `i` shows `news.slice(i * 6, i * 6 + 6)` in the same order as `#news` (story `date` descending). Last page may have **1–5** cards (no padding, no duplicates). Loop: last → first.
3. **Idle / short list:** If `n <= 6`, keep today’s static grid. No carousel chrome, no timers.
...
11. **`prefers-reduced-motion`:** No autoplay, no fade, no pause button. First six + View all.
...
1. Subtle disabled/hidden pause button when `n <= 6` rather than a layout hole (header stays aligned with View all).
...
- Grid looks like today’s news cards; only the six (or fewer on the last page) swap. Short last pages keep **natural card height** (no stretched cards, no forced grid min-height).
...
- Do not lock the grid `min-height` to a six-card page; short last pages keep intrinsic card size (`align-items: start`).
- Existing `#home-news-grid` id stays (router alias + smoke A4).
- Fade via CSS + class toggle; no new libraries.

## 8. Success metrics

- With 39 items: 7 pages (6+6+6+6+6+6+3); loops; last page has three cards at the same card size as a full page.
...
- Reduced-motion: static six, no button, View all works.
- `#news` still lists all items with search/year chips.

## 9. Open questions

- None blocking. Count will grow past 39; paging is `ceil(n / 6)` with no cap (SPA list pagination ~200-row trigger stays deferred and does **not** apply to this Home teaser).
